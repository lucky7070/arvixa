        ...
        $('.board-id, .consumer-no').on('change keyup', function () {
            ...
            if (board_id !== "" && isValidLength) {
                ...
                $.ajax({
                    ...
                    complete: function () {
                        $('.electricity-btn').prop('disabled', false).text('Get Details');
                    }
                });
    ...
    $(document).ready(function () {
        ...
        $('#confirmPayBtn').on('click', function () {
            ...
            $.ajax({
                ...
                data: formData,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // make sure CSRF token meta tag is present
                },
                success: function (response) {
                    $('.success-message').html(response.message || 'Electricity bill paid successfully!');
                    $('#successModal').modal('show');
                },
                error: function (xhr) {
                    let errorMsg = xhr.responseJSON?.message || 'Something went wrong. Please try again.';
                    $('.consumer-message').html('<span class="text-danger">' + errorMsg + '</span>');
                }
            });
        });
    });
