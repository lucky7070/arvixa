...
<div class="card">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('retailer.cart') }}" class="btn btn-outline-secondary">
    ...
    <div class="card-body">
        <div class="row">
            <div class="col-lg-5 order-md-last">
                ...
                <ul class="list-group mb-3">
                    ...
                    <li class="list-group-item d-flex justify-content-between lh-sm">
                        <a href="{{ route('retailer.product_details', @$row->product->slug) }}">
                            <h6 class="my-0">{{ @$row->product->name }} x {{ $row->qty }}</h6>
                            <small class="text-secondary fw-bold me-1 border-right">
                                Brand - {{ @$row->product->brand->name }}
                            </small>
                            <small class="text-danger fw-bold me-1">
                                Item Price -
            ...
            <div class="col-lg-7">
                ...
                <form id="shippingAddress" method="post" action="{{ route( 'retailer.place-order' ) }}">
                    @csrf
                    <div class="row g-3">
                        <div class="col-sm-6">
                            ...
                            <input type="text" name="first_name" class="form-control" id="first_name"
                                placeholder="First Name" value="{{ old('first_name', @$lastOrder->customer_name_1) }}"
                                required="">
                            @error('first_name')
                        ...
                        <div class="col-12">
                            ...
                            <input type="email" name="email" class="form-control" id="email"
                                placeholder="you@example.com" value="{{ old('email', @$lastOrder->customer_email) }}">
                            @error('email')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" name="address" class="form-control" id="address"
                                placeholder="1234 Main St" value="{{ old('address', @$lastOrder->shipping_address_1) }}"
                                required="">
                            @error('address')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
...
<script>
    $(function () {
        ...
        function placeOrder(form) {
            ...
            $.ajax({
                url: "{{ route('retailer.place-order') }}",
                data: formData,
                contentType: false,
                processData: false,
                type: 'POST',
                success: function (data) {
                    if (data.status) {
                        ...
                        swal({
                            ...
                            timer: 15000,
                            button: "Okay",
                        }).then(() => window.location.href = "{{ route('retailer.my-orders') }}")
