...
<div class="card mb-3">
    <div class="card-header">
        <div class="row flex-between-end">
            <div class="col-auto align-self-center">
                <h5 class="mb-0" data-anchor="data-anchor">Vouchers :: Voucher Edit </h5>
            </div>
    ...
    <div class="card-body">
        <form class="row" id="ediUser" method="POST" action="{{ route('vouchers.edit', $voucher['id']) }}"
            enctype='multipart/form-data'>
            @csrf
            <div class="row">
                <div class="col-md-6 mb-2">
                    <label class="form-label" for="type">Voucher Type
                        <span class="text-danger">*</span>
                    </label>
                    <select name="type" class="form-select" id="type">
                        @foreach (config('constant.voucher_type_list', []) as $key => $value)
                        <option name="type" value="{{ $key }}" @selected(old('type', $voucher->type)==$key)>
                            {{ $value }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-2">
                    <div class="form-group">
                        <label class="form-label" for="name">Coupon Name
                            <span class="text-danger">*</span>
                        </label>
                        <input type="text" name="name" id="name" class="form-control"
                            value="{{ old('name', $voucher->name) }}" placeholder="Name of the coupon">
                    </div>
                    @error('name')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                ...
                <div class="col-md-4 mb-2">
                    <div class="form-group">
                        <label class="form-label" for="code">Coupon Code
                            <span class="text-danger">*</span>
                        </label>
                        <input type="text" id="code" name="code" minlength="5" maxlength="12" class="form-control"
                            placeholder="Promotion code or Generate one" value="{{ old('code', $voucher->code) }}">
                    </div>
                    @error('code')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                ...
                <div class="col-md-6 mb-2">
                    <div class="form-group">
                        ...
                        <input type="number" name="max_uses" id="max_uses" class="form-control"
                            placeholder="Total Usage limit" min="1" value="{{ old('max_uses', $voucher->max_uses) }}">
                    </div>
                    @error('max_uses')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                ...
                <div class="col-md-6 mb-2">
                    <div class="form-group">
                        <label for="max_uses_per_user">Usage limit per User
                            <span class="text-danger">*</span>
                        </label>
                        <input type="number" name="max_uses_per_user" id="max_uses_per_user" class="form-control"
                            placeholder="Usage limit per user" min="1"
                            value="{{ old('max_uses_per_user', $voucher->max_uses_per_user) }}">
                    </div>
                ...
                <div class="col-md-6 mb-2">
                    <div class="form-group">
                        <label class="form-label" for="starts_at">From Date
                            <span class="text-danger">*</span>
                        </label>
                        <input type="datetime-local" name="starts_at" id="starts_at" class="form-control"
                            value="{{ old('starts_at', $voucher->starts_at) }}">
                    </div>
                    @error('starts_at')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                ...
                <div class="col-md-6 mb-2">
                    <div class="form-group">
                        <label class="form-label" for="expires_at">To Date
                            <span class="text-danger">*</span>
                        </label>
                        <input type="datetime-local" name="expires_at" id="expires_at" class="form-control"
                            value="{{ old('expires_at', $voucher->expires_at) }}">
                    </div>
                    @error('expires_at')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                ...
                <div class="col-md-4 mb-2">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="is_fixed" name="is_fixed">
                            <option value="1" @selected(old('is_fixed', $voucher->is_fixed) == 1)>Flat Discount</option>
                            <option value="2" @selected(old('is_fixed', $voucher->is_fixed) == 2)>Percentage Discount
                            </option>
                        </select>
                    </div>
                    @error('is_fixed')
                ...
                <div class="col-md-4 mb-2">
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="discount_amount" name="discount_amount"
                            ...
                            value="{{ old('discount_amount', $voucher->discount_amount) }}" min="1">
                    </div>
                ...
                <div class="col-md-4 mb-2">
                    <div class="form-group">
                        ...
                        <input type="number" name="max_discount" id="max_discount" class="form-control max_discount"
                            ...
                            placeholder="Maximum Discount to apply.">
                    </div>
                    @error('max_discount')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                ...
                <div class="col-lg-3 col-md-4 mb-2">
                    <div class="form-group">
                        ...
                        <input type="number" name="min_cart_value" id="min_cart_value" class="form-control"
                            value="{{ old('min_cart_value', $voucher->min_cart_value) }}" min="1"
                ...
                <div class="col-lg-3 col-md-4 mb-2">
                    <div class="form-group">
                        ...
                        <select name="is_free_shipping" class="form-select" id="is_free_shipping">
                            <option value="0" @selected(old('is_free_shipping', $voucher->is_free_shipping)==0)> No
                            </option>
                            <option value="1" @selected(old('is_free_shipping', $voucher->is_free_shipping)==1)> Yes
                            </option>
                        </select>
                    </div>
                ...
                <div class="col-lg-3 col-md-4 mb-2">
                    <div class="form-group">
                        ...
                        <select name="status" class="form-select" id="status">
                            <option value="1" @selected(old('status', $voucher->status)==1)> Active</option>
                            <option value="0" @selected(old('status', $voucher->status)==0)> Inactive</option>
                        </select>
                    </div>
                    @error('status')
                    <span class="invalid-feedback" role="alert">
                ...
                <div class="col-lg-3 col-md-4 mb-2">
                    <div class="form-group">
                        ...
                        <select name="is_public" class="form-select" id="is_public">
                            <option value="1" @selected(old('is_public', $voucher->is_public)==1)> Yes</option>
                            <option value="0" @selected(old('is_public', $voucher->is_public)==0)> No</option>
                        </select>
                    </div>
                    @error('is_public')
                    <span class="invalid-feedback" role="alert">
...
<script type="text/javascript">
    $(function () {
        ...
        let showMaxDiscount = "{{ old('max_discount', $voucher->max_discount) }}";
        if (showMaxDiscount) {
            $(".max_discount").show();
        } else {
            $(".max_discount").hide();
