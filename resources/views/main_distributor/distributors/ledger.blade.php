    ...
    <div class="card-header">
        <div class="row flex-between-end">
            <div class="col-auto align-self-center">
                <h5 class="mb-0" data-anchor="data-anchor">Distributors :: Ledger - <span class="text-primary">{{
                        $user['name'] }} ( ₹ {{ $user['user_balance'] }})</span> </h5>
            </div>
            <div class="col-auto ms-auto">
                <div class="nav nav-pills nav-pills-falcon">
                    <a href="{{ route('main_distributor.distributors')  }}" class="btn btn-outline-secondary me-2">
                        <i class="fa fa-arrow-left me-1"></i>
                        Go Back
                    </a>
                    <a href="{{ route('ledger.export', ['user' => $user['slug'], 'user_type' => 3 ])  }}"
...
<script type="text/javascript">
    $(function () {
        var table = $('.table-datatable').DataTable({
            ajax: "{{ route('main_distributor.distributors.ledger', $user['slug']) }}",
            order: [
                [1, 'desc']
            ],
            columns: [{
        ...
        $("#addForm").validate({
            ...
            submitHandler: function (form) {
                ...
                $("#overlay").show();
                $.ajax({
                    url: "{{ route('main_distributor.distributors.ledger', $user['slug']) }}",
                    data: formData,
                    contentType: false,
                    processData: false,
                    type: 'POST',
