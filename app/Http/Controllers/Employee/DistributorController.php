...
class DistributorController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return Datatables::of($query)->addIndexColumn()
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<a class="dropdown-item" href="' . route('employee.distributors.edit', $row['slug']) . '">Edit</a>';
                    $btn .= '<a class="dropdown-item" href="' . route('employee.distributors.services', $row['slug']) . '">Services</a>';
                    $btn .= '<a class="dropdown-item" href="' . route('employee.retailers', ['distributor' => $row->id]) . '">Retailers</a>';
                    $btn .= '<a class="dropdown-item" href="' . route('employee.distributors.ledger', $row['slug']) . '">Ledger</a>';
                    $btn .= '</div>';
        ...
        return view('employee.distributors.index');
    }

    public function add()
    {
        $main_distributors = MainDistributor::select('id', 'name')->where('status', 1)->get();
        return view('employee.distributors.add', compact('main_distributors'));
    }
    ...
    public function save(Request $request)
    {
        ...
        return redirect(route('employee.distributors'))->with('success', 'Distributor Added Successfully!!');
    }

    public function edit($id)
    {
        $distributor = Distributor::where('slug', $id)->where('employee_id', auth()->guard('employee')->id())->first();
        if ($distributor == null) {
            return redirect(route('employee.distributors'))->with('error', 'Distributor Not Found!!');
        }

        $main_distributors = MainDistributor::select('id', 'name')->where('status', 1)->get();
        return view('employee.distributors.edit', compact('distributor', 'main_distributors'));
    }

    public function update(Request $request, $id)
    {
        $distributor = Distributor::where('id', $id)->where('employee_id', auth()->guard('employee')->id())->first();
        if ($distributor == null) {
            return redirect(route('employee.distributors'))->with('error', 'Distributor Not Found!!');
        }
        ...
        return redirect(route('employee.distributors'))->with('success', 'Distributor Updated Successfully!!');
    }

    public function delete(Request $request)
    {
        if ($request->id) {
            $distributor = Distributor::where('id', $request->id)->where('employee_id', auth()->guard('employee'))->first();
            if ($distributor == null) {
    ...
    {
        $user = Distributor::with(['main_distributor'])->where('slug', $slug)->where('employee_id', auth()->guard('employee')->id())->first();
        if ($user == null) {
            return redirect(route('employee.distributors'))->with('error', 'Distributor Not Found!!');
        }
        ...
        return view('employee.distributors.services', compact(['user', 'services']));
    }

    public function services_update(Request $request, $slug)
    {
        $distributor = Distributor::where('slug', $slug)->where('employee_id', auth()->guard('employee')->id())->first();
        if (!$distributor) {
            return response()->json([
                'status'   => false,
                'message'   => 'Distributor Not Found.',
    ...
    public function ledger(Request $request, $slug)
    {
        ...
        return view('employee.distributors.ledger', compact('user'));
    }
