...
class ElectricityController extends Controller
{
    ...
    //             ->where('bill_type', 'electricity') // ✅ Only electricity bills
    //             ->paginate(5);
                
               
    //     return view('retailer.electricity', compact('bills'));
    // }
 ...
 public function view()
{
    ...
    $commission = ServicesLog::where('user_id', $retailer->id)
        ->where('service_id', 8) // 8 is the electricity service ID
    ...
    if (!$commission) {
        return redirect()->back()->with('error', 'Electricity service is not active for your account.');
    }
    ...
    public function electricityGetDetails(Request $request) {
    ...
    public function electricityPaymentSubmit(Request $request)
    {
        $retailer = auth()->user();
        ...
        try {
            ...
            $paymentTransaction->particulars = "Electricity bill paid for Consumer No.: " . $request->consumer_no;
            ...
            if ($commission) {
                ...
                $commissionTransaction->particulars = "Electricity Bill Service Commission credited for Consumer No.: " . $request->consumer_no;
